        ...
        success: function (response) {
          ...
          if(response){
            var data =JSON.parse(response);
              var html = '<option>Select</option>';
              jQuery.each(data, function( i, val ) {
                html +='<option value="'+val.id+'">'+val.unit+'</option>';
              });
              $('#unit_id').html(html);
          }
        },
        ...
        success: function (response) {
          ...
          if(response){
            var data =JSON.parse(response);
              var html = '<option>Select</option>';
              jQuery.each(data, function( i, val ) {
                html +='<option value="'+val.id+'">'+val.subunit_name+'</option>';
              });
              $('#subunit_id').html(html);
          }
        },
        error: function(jqXHR, textStatus, errorThrown) {
           console.log(textStatus, errorThrown);
...
$('.status').click(function(){
  ...
  $.ajax({
        ...
        success: function (response) {
           ...
           if(response){
            $('.danger').hide();
            $('.success').html('User status changed successfully');
            $('.success').show();
            setTimeout(function(){
              ...
              $('.success').hide();
              $('.danger').html('User status not changed');
              $('.danger').show();
           }
        ...
        success: function (response) {
          ...
          if(response){
            ...
            switch(data[0].answer){
              case 'option_a':
                answer = 'Option A: '+data[0].option_a;
              break;
              case 'option_b':
                answer = 'Option B: '+data[0].option_b;
              break;
              case 'option_c':
                answer = 'Option C: '+data[0].option_c;
              break;
              case 'option_d':
                answer = 'Option D: '+data[0].option_d;
              break;
              case 'option_e':
                answer = 'Option E: '+data[0].option_e;
              break;
            }
            var html='<table class="table table-responsive-xl table-striped">'+
                        ...
                        '<tr>'+
                          ...
                          '<td>'+data[0].subunit_name+'</td>'+
                        '</tr>'+
                        '<tr>'+
                          '<th scope="row">Question Type</th>'+
                          '<td>'+data[0].question_type+'</td>'+
                         '</tr>'+
                        '<tr>'+
                          '<th scope="row">Question</th>'+
                        ...
                        '<tr>'+
                          ...
                          '<td>'+data[0].option_a+'</td>'+
                        '</tr>'+
                        '<tr>'+
                          '<th scope="row">Option B</th>'+
                          '<td>'+data[0].option_b+'</td>'+
                        '</tr>';
                        
                        if(data[0].option_c !=''){
                        html+='<tr>'+
                          '<th scope="row">Option C</th>'+
                          '<td>'+data[0].option_c+'</td>'+
                        '</tr>';
                        }
                        if(data[0].option_d !=''){
                          html+='<tr>'+
                            '<th scope="row">Option D</th>'+
                            '<td>'+data[0].option_d+'</td>'+
                          '</tr>';
                        }
                        if(data[0].option_e !=''){
                          html+='<tr>'+
                            '<th scope="row">Option E</th>'+
                            '<td>'+data[0].option_e+'</td>'+
                          '</tr>';
                        }

                        html+='<tr>'+
                          '<th scope="row">Explaination</th>'+
                          '<td>'+data[0].explaination+'</td>'+
                        '</tr>'+
                        '<tr>'+
